...
void print_list(set<Segment*, function<bool(Segment*, Segment*)>> lst) {
    for (set<Segment*, function<bool(Segment*, Segment*)>>::iterator it = lst.begin(); it != lst.end(); ++it) {
        printf("\n\t");
        print_seg(*it);
    }
    printf("\nKoniec listy.\n");
...
//        }
        ...
        return (min(s->left->y, s->right->y)); // to się da konkretnie wskazać.
    }
}
...
int sweep_x = 0;
bool seg_cmp(Segment * s1, Segment * s2) {
	if (DEBUG) {
		printf("*** Porownuje "); 
		print_seg(s1);
		printf(" i ");
	...
	if (check_cross_with_line(s1, sweep_x) == check_cross_with_line(s2, sweep_x)) {
		if (DEBUG) { printf("A%d\n", (s1->dir < s2->dir)); }
		return (s1->dir < s2->dir);
	} else {
		if (DEBUG) { printf("B%d\n", (check_cross_with_line(s1, sweep_x) < check_cross_with_line(s2, sweep_x))); }
		return (check_cross_with_line(s1, sweep_x) < check_cross_with_line(s2, sweep_x));
	}
}
...
int main() {
    ...
    set<Segment*, function<bool(Segment*, Segment*)>> current(seg_cmp);
    ...
    Point * hp;
    Event * he;
    set<Segment*, function<bool(Segment*, Segment*)>>::iterator it2;
    set<Segment*, function<bool(Segment*, Segment*)>>::iterator it_c;
    while (!eventq.empty()) {
	e = NULL;
	seg_count = 0;
        tab_clear(new_segs);
        ...
        do {
            ...
            if (e->seg != NULL) {
		if (e->seg->dir == 3) {
			if (vseg != NULL && point_cmp(vseg->right, e->seg->left) == 0) {
				seg_count++;
			}	
			vseg = e->seg;
			// jakis lower_bound czy cos
			for (set<Segment*, function<bool(Segment*, Segment*)>>::iterator it = current.begin(); it != current.end(); ++it) {
				hp = check_segment_cross(*it, vseg);
				if (hp != NULL) {
				    he = (Event *) malloc(sizeof(Event));
				    he->p = hp;
				    he->seg = NULL;
				    eventq.push(he);
				    if (DEBUG) { printf("Wrzucam event: "); print_event(he); printf("\n"); }
				}	
			}
	...
	sweep_x = p->x;
	if (vseg != NULL && point_cmp(vseg->right, p) < 0) {
		vseg = NULL;
	}
        bool it2_inserted = false;
        it2 = current.end();
	bool begin_nice_segments = false;
        // if (DEBUG) { printf("Wygląd listy przed wyjeciem:\n"); print_list(current); }
        //for (set<Segment*, function<bool(Segment*, Segment*)>>::iterator it = current.begin(); it != current.end(); ++it) {
	//	if (DEBUG) { printf("1. Iterator wskazuje na: "); if (it == current.end()) { printf("NOPE"); } else { print_seg(*it); }; printf("\n"); }
        //    if (point_cmp((*it)->right, p) == 0) {
	//	if (DEBUG) { printf("Punkt jest z brzegu, wyjmujemy!\n"); }
        //        end_segs[(*it)->dir] = *it;
		//it_c = it;
		//it_c--;
        //        it = current.erase(it);
        //        it2 = it;
        //        it2_inserted = true;
        //        if (it == current.end()) { break; } else { it--; /*it = it_c;*/ }
	//	begin_nice_segments = true;
        //    } else if (is_between(*it, p)) {
	//	if (DEBUG) { printf("Punkt zawiera sie w segmencie, wyjmujemy!\n"); }
         //       mid_segs[(*it)->dir] = *it;
		//it_c = it;
		//it_c--;
           //     it = current.erase(it);
	//	if (DEBUG) { printf("2. Iterator wskazuje na: "); if (it == current.end()) { printf("NOPE"); } else { print_seg(*it); }; printf("\n"); }
        //        it2 = it;
        //        it2_inserted = true;
        //        if (it == current.end()) { break; } else { it--; }
	//	begin_nice_segments = true;
        //    } else if (begin_nice_segments) {
	//	if (DEBUG) { printf("Koniec fajnych punktow, wypad\n"); }
	//	break;
	//    }
	//	if (DEBUG) { printf("3. Iterator wskazuje na: "); if (it == current.end()) { printf("NOPE"); } else { print_seg(*it); }; printf("\n"); }
        //}
	//
	if (DEBUG) { printf("DO ROBOTY!\n"); }
	it2 = current.begin();
	// tutaj dalej debilu przegladasz liniowo
	while (it2 != current.end()) {
		if (((point_cmp((*it2)->right, p) == 0) || (is_between(*it2, p))) || (((float) p->y) < check_cross_with_line(*it2, p->x))) {
			break;
		}
		it2++;
	}
	if (DEBUG) { printf("Wstępnie Iterator wskazuje na: "); if (it2 == current.end()) { printf("END"); } else { print_seg(*it2); }; printf("\n"); }

	if (it2 != current.end() && ((point_cmp((*it2)->right, p) == 0) || (is_between(*it2, p)))) {
		while (it2 != current.end() && ((point_cmp((*it2)->right, p) == 0) || (is_between(*it2, p)))) { 
			if (point_cmp((*it2)->right, p) == 0) {
				if (DEBUG) { printf("Punkt jest z brzegu, wyjmujemy!\n"); }
				end_segs[(*it2)->dir] = *it2;
				it2 = current.erase(it2);
				if (DEBUG) { printf("2. Iterator wskazuje na: "); if (it2 == current.end()) { printf("NOPE"); } else { print_seg(*it2); }; printf("\n"); }
				it2_inserted = true;
			} else if (is_between(*it2, p)) {
				if (DEBUG) { printf("Punkt zawiera sie w segmencie, wyjmujemy!\n"); }
				mid_segs[(*it2)->dir] = *it2;
				it2 = current.erase(it2);
				if (DEBUG) { printf("2. Iterator wskazuje na: "); if (it2 == current.end()) { printf("NOPE"); } else { print_seg(*it2); }; printf("\n"); }
				it2_inserted = true;
			}
       ...
       // }
        ...
        it2_inserted = true;
	if (vseg != NULL) { seg_count++; }
        if ((seg_count + count_not_nulls(new_segs) + count_not_nulls(mid_segs) + count_not_nulls(end_segs)) > 1) {
            if (DEBUG) {
	...
	bool left_inserted = false;
        ...
        set<Segment*, function<bool(Segment*, Segment*)>>::iterator it_right;
        set<Segment*, function<bool(Segment*, Segment*)>>::iterator it_left;
        set<Segment*, function<bool(Segment*, Segment*)>>::iterator it3;
        for (int i=0; i < 4; i++) {
            if (new_segs[i] != NULL) {
